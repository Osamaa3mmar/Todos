...
export default function LanguageButton() {
  ...
  return (
    ...
    <ul className=" dropdown-center  dropdown menu  w-30 max-h-[400px] overflow-auto flex-nowrap rounded-box bg-base-100 shadow-sm"
      ...
      <li key={"arabic"} className={currentLang=="ar"?" text-white rounded-md bg-primary ":null}  onClick={()=>{changeLanguage("ar")}}> <a><p className=' font-bold text-xs'>Ar</p> <p className=''>Arabic</p></a></li>
      <li key={"english"} className={currentLang=="en"?" text-white rounded-md bg-primary  ":null}  onClick={()=>{changeLanguage("en")}}> <a><p className=' font-bold text-xs'>En</p> <p className=''>English</p></a></li>
    </ul>
    </div>
  )
}
